(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.jltmrt-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.jltmrt-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.jltmrt-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltmrt-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTMRTCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jltmrt_recommended_activate_plugin",
            file: file,
            nonce: JLTMRTCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jltmrt_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTMRTCORE.recommended_nonce
        },
