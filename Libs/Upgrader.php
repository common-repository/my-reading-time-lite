...
if ( ! class_exists( 'Upgrader' ) ) {
	...
	class Upgrader {
		...
		public function __construct() {
			$this->option_name = JLT_My_Reading_Time::plugin_version_key();
		}
		...
		protected function get_installed_version() {
			return get_option( $this->option_name, JLTMRT_VER );
		}
		...
		public function if_updates_available() {
			if ( version_compare( $this->get_installed_version(), JLTMRT_VER, '<' ) ) {
				return true;
			}
		...
		public function run_updates() {
			...
			$path              = trailingslashit( JLTMRT_DIR );
